...
abc1=function (target, param, sumstat, tol, method, hcorr = TRUE, 
          transf = "none", logit.bounds = c(0, 0), subset = NULL, kernel = "epanechnikov", iteration=10,numnet = 10, sizenet = 5, lambda = c(1e-04, 0.001, 0.01), trace = FALSE, maxit = 500, hidden=c(30,30), activationfun="sigm", learningrate=0.8 ,momentum=0.5, learningrate_scale=1, output="sigm", sae_output="sigm", numepochs=5,hidden_dropout=0.1, visible_dropout=0.1,...)
{
  call <- match.call()
  ...
  if (!any(method == c("rejection", "loclinear", "neuralnet", 
                       "ridge","sae","none","db"))) {
    stop("Method must be rejection, loclinear, or neuralnet, ridge, none, sae or db")
  }
  if (method == "rejection") 
    rejmethod <- TRUE
  ...
  if (rejmethod) {
    ...
    if (method == "neuralnet") {
      ...
      for (i in 1:numnet) {
        fit1 <- nnet(scaled.sumstat[wt1, ], param[wt1, ], weights = weights, decay = lambda[i], size = sizenet, 
  ...
  if (method=="none") {
    ...
    for (i in 1:iteration) {
     ...
     #                    momentum, learningrate_scale, output, numepochs, batchsize = sum(wt1), hidden_dropout , visible_dropout)
      ...
      fit0=nn.train(scaled.sumstat[wt1,], param[wt1,],initW=weights, hidden=c(30,30), activationfun="sigm", learningrate=0.8,
    ...
    if(hcorr == TRUE){
      ...
      for (i in 1:iteration){
         ...
         ##              momentum, learningrate_scale, output, numepochs, batchsize = sum(wt1), hidden_dropout, visible_dropout)
        fit1=nn.train(scaled.sumstat[wt1,], log(residuals^2),initW=weights, hidden=c(30,30), activationfun="sigm", learningrate=0.8,
                      momentum=0.5, learningrate_scale = 1, output="sigm", numepochs=100, batchsize = sum(wt1), hidden_dropout=0.1, visible_dropout=0.1)
      ...
      #  fit2=nn.train(scaled.sumstat[wt1,], log(residuals0^2), initW=weights,hidden, activationfun, learningrate,
       #              momentum, learningrate_scale, output, numepochs, batchsize = sum(wt1), hidden_dropout, visible_dropout)
        fit2=nn.train(scaled.sumstat[wt1,], log(residuals0^2), initW=weights,hidden=c(30,30), activationfun="sigm", learningrate=0.8,
                      momentum=0.5, learningrate_scale = 1, output="sigm", numepochs=100, batchsize = sum(wt1), hidden_dropout=0.1, visible_dropout=0.1)
        cat(i)
        fv2[,,i]=fit1$post[[4]]
        residuals2[,,]=fit1$e
  ...
  if(method == "rejection"){
    ...
    out <- list(adj.values=adj.values, unadj.values=unadj.values,
      ...
      out <- list(adj.values=adj.values, unadj.values=unadj.values,
                  ss=ss, weights=weights, residuals=residuals, dist=dist,
                  call=call, na.action=gwt, region=wt1, transf=transf, logit.bounds = logit.bounds,
                  method="none", hcorr = hcorr,  numparam=numparam, numstat=nss,
                  names=list(parameter.names=paramnames, statistics.names=statnames))
   #   iteration=iteration, 
   #   hidden =hidden, activationfun = activationfun, learningrate = learningrate,learningrate_scale=learningrate_scale,
    #  momentum = momentum,  output = output, numepochs =numepochs, hidden_dropout =hidden_dropout, 
      }
        ...
        out <- list(adj.values=adj.values, unadj.values=unadj.values,
                    ss=ss, weights=weights, residuals=residuals, dist=dist,
                    call=call, na.action=gwt, region=wt1, transf=transf, logit.bounds = logit.bounds,
                    method="sae", hcorr = hcorr,  numparam=numparam, numstat=nss,
                    names=list(parameter.names=paramnames, statistics.names=statnames))
  }
else if(method == "db"){
          out <- list(adj.values=adj.values, unadj.values=unadj.values,
                      ss=ss, weights=weights, residuals=residuals, dist=dist,
                      call=call, na.action=gwt, region=wt1, transf=transf, logit.bounds = logit.bounds,
                      method="db", hcorr = hcorr,  numparam=numparam, numstat=nss,
                      names=list(parameter.names=paramnames, statistics.names=statnames))
    }
