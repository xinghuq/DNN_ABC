...
cvabctest=cv4abc(as.data.frame(b),c,abc.out = NULL,nval=10, tols=0.3, statistic = "median", prior.range = NULL,
       method="ridge", hcorr = TRUE,
       transf = "none", logit.bounds = c(0,0), subset = NULL, kernel = "epanechnikov",
       numnet = 10, sizenet = 5, lambda = c(0.0001,0.001,0.01), trace = FALSE, maxit = 100, hidden_dropout=0.1, visible_dropout=0.1)

cv4abc <- function(param, sumstat, abc.out = NULL, nval, tols,
                   statistic = "median", prior.range = NULL,
                   method, hcorr = TRUE,
                   transf = "none", logit.bounds = c(0,0), subset = NULL, iteration=10,kernel = "epanechnikov",
                   numnet = 10, sizenet = 5, lambda = c(0.0001,0.001,0.01), trace = FALSE, maxit = 500, hidden=c(30,30), activationfun="sigm", learningrate=0.8,
                   momentum=0.5, learningrate_scale=0.8, output="sigm", sae_output="sigm", numepochs=5, hidden_dropout=0.1, visible_dropout=0.1,...){
  ...
  for(mytol in tols){
    ...
    for(i in 1:nval){
      ...
      subres <- withCallingHandlers( abc1(target = mytarget, param = myparam, sumstat = mysumstat, tol=mytol,
                                         subset = mysubset, method = method, transf = transf, logit.bounds = logit.bounds, kernel = kernel,hcorr = hcorr,iteration=iteration,hidden=hidden, activationfun=activationfun, learningrate=learningrate ,
                                         momentum=momentum , learningrate_scale=learningrate_scale, output=output, sae_output=sae_output, numepochs=numepochs, hidden_dropout=hidden_dropout, visible_dropout=visible_dropout), warning = namesWarningFilter)
    ...
    mycall[[paste("tol", mytol, sep="")]] <- call("abc1", target = quote(target), param = quote(param), sumstat = quote(sumstat),
                                                  tol= mytol,
                                                  subset = quote(subset), method = subres$method, hcorr = subres$hcorr,transf = subres$transf,
                                                  logit.bounds = subres$logit.bounds, kernel = subres$kernel,iteration=iteration,hidden=hidden, activationfun=activationfun, learningrate=learningrate ,
                                                  momentum=momentum , learningrate_scale=learningrate_scale, output=output, sae_output=sae_output, numepochs=numepochs, hidden_dropout=hidden_dropout, visible_dropout=visible_dropout)
  }
  if(np==1){
